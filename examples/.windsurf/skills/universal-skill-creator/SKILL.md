...
- Do not invent platform minimum versions; use `unknown` when not verified and document the production routing risk.
...
- Duplicate platform copies are synced from the canonical source or their divergence is documented.

## Output Requirements

- Provide changed skill paths, canonical source used, version changes, and changelog entries.
- List schema and content-readiness validation performed.
- Call out unknown platform compatibility and required human review.
...
- Set `min_platform_version` for every target platform supported by the repository. Use `"unknown"` if the minimum version is not known; do not omit configured platforms or invent versions.
- Set `replaces` if this skill directly succeeds a previously named skill.
- Set `supersedes` if this skill functionally replaces multiple older skills.
...
11. Skill-specific operating model, checklist, decision rules, acceptance criteria, and output requirements are present and domain-specific.
